...
const pLimit = require('p-limit'); // Requires npm install p-limit@3

// Centralized Configuration
const CONFIG = {
  TIMEOUT: 6000, // Standard timeout (6s) for heavy sites or Cloudflare-protected ones
  TIMEOUT_API: 3000, // Reduced timeout (3s) for fast APIs (e.g., Knaben, TPB)
...
// Optional dynamic tracker updater
async function updateTrackers() {
  try {
    ...
    // Silent failure, fallback to static list
  }
}

// Strict timeout wrapper
const withTimeout = (promise, ms) =>
  Promise.race([promise, new Promise((_, reject) => setTimeout(() => reject(new Error('Engine Timeout')), ms))]);

const httpsAgent = new https.Agent(CONFIG.HTTPS_AGENT_OPTIONS);

// Cloudflare bypass wrapper
async function cfGet(url, config = {}) {
  const headers = { 'User-Agent': getRandomUA(), ...config.headers };
  try {
    return await axios.get(url, { ...config, headers, httpsAgent });
  } catch {
    try {
      const html = await cloudscraper.get(url, { headers, timeout: CONFIG.TIMEOUT });
      return { data: html };
    } catch {
      return { data: '' };
    }
...
/**
 * Cleans and normalizes a title (Improved version)
 */
function clean(title) {
  if (!title) return '';
  // Decode common HTML entities
  const htmlDecoded = title
    .replace(/&amp;/g, '&')
    .replace(/&lt;/g, '<')
    .replace(/&gt;/g, '>')
    .replace(/&quot;/g, '"')
    .replace(/&#039;/g, "'")
    .replace(/&apos;/g, "'");
  // Remove problematic characters but preserve () and []
  return htmlDecoded
    .replace(/[:"'’]/g, '')
...
/**
 * Checks if title indicates Italian content (Improved version)
 */
function isItalianResult(name) {
  if (!name) return false;
  const upperName = name.toUpperCase();
  // If only ENG without ITA/MULTI
  if (/\b(ENG|ENGLISH)\b/i.test(upperName) && !/\b(ITA|MULTI|DUAL)\b/i.test(upperName)) return false;
  // Expanded Italian pattern
  const ITA_REGEX =
  ...
  return ITA_REGEX.test(upperName);
}

/**
 * Checks if year in title matches (Improved version)
 */
...
/**
 * Parses IMDb ID (Improved version)
 */
...
async function searchCorsaro(title, year, type, reqSeason, reqEpisode) {
  try {
    ...
    $('a').each((i, elem) => {
      ...
      if (!isItalianResult(text) || !checkYear(text, year, type) || !isCorrectFormat(text, reqSeason, reqEpisode))
        return;
...
async function searchLime(title, year, type, reqSeason, reqEpisode) {
  try {
    ...
    const promises = candidates.slice(0, 5).map((cand) =>
      limit(async () => {
        try {
...
async function searchMagnet(title, year, type, imdbId) {
  // await updateTrackers(); // Uncomment if dynamic updates are needed
  ...
  const promises = CONFIG.ENGINES.map((engine) => {
    ...
    return withTimeout(engine(title, year, type, reqSeason, reqEpisode), specificTimeout).catch(() => []);
  });
